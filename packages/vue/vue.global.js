var Vue = (function (exports) {
  ...
  function processFor(node, dir, context, processCodegen) {
    if (!dir.exp) {
      context.onError(
        createCompilerError(29 /* X_V_FOR_NO_EXPRESSION */, dir.loc)
      );
      return;
    }
    ...
    if (!parseResult) {
      context.onError(
        createCompilerError(30 /* X_V_FOR_MALFORMED_EXPRESSION */, dir.loc)
      );
      return;
    }
  ...
  function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (onComponentSlot) {
        ...
        context.onError(
          createCompilerError(34 /* X_V_SLOT_MIXED_SLOT_USAGE */, slotDir.loc)
        );
        break;
      }
      ...
      if ((vIf = findDir(slotElement, "if"))) {
        ...
        if (prev && isTemplateNode(prev) && findDir(prev, "if")) {
          ...
          context.onError(
            createCompilerError(28 /* X_V_ELSE_NO_ADJACENT_IF */, vElse.loc)
          );
        }
        ...
        if (parseResult) {
          ...
          context.onError(
            createCompilerError(30 /* X_V_FOR_MALFORMED_EXPRESSION */, vFor.loc)
          );
        }
      } else {
        ...
        if (staticSlotName) {
          if (seenSlotNames.has(staticSlotName)) {
            context.onError(
              createCompilerError(
                35 /* X_V_SLOT_DUPLICATE_SLOT_NAMES */,
                dirLoc
              )
            );
            continue;
    ...
    for (let i = 0; i < props.length; i++) {
      ...
      if (prop.type === 6 /* ATTRIBUTE */) {
        ...
        if (name === "slot") {
          if (!isComponent) {
            context.onError(
              createCompilerError(37 /* X_V_SLOT_MISPLACED */, loc)
            );
          }
          continue;
        }
  ...
  function processSlotOutlet(node, context) {
    ...
    if (propsWithoutName.length > 0) {
      ...
      if (directives.length) {
        context.onError(
          createCompilerError(
            33 /* X_V_SLOT_UNEXPECTED_DIRECTIVE_ON_SLOT_OUTLET */,
            directives[0].loc
          )
        );
      }
  ...
  const transformBind = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createCompilerError(31 /* X_V_BIND_NO_EXPRESSION */, loc)
      );
    }
  ...
  const transformModel = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createCompilerError(38 /* X_V_MODEL_NO_EXPRESSION */, dir.loc)
      );
    ...
    if (!isMemberExpression(expString)) {
      context.onError(
        createCompilerError(39 /* X_V_MODEL_MALFORMED_EXPRESSION */, exp.loc)
      );
      return createTransformProps();
    }
  ...
  const transformVHtml = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(47 /* X_V_HTML_NO_EXPRESSION */, loc)
      );
    }
    if (node.children.length) {
      context.onError(
        createDOMCompilerError(48 /* X_V_HTML_WITH_CHILDREN */, loc)
      );
  ...
  const transformVText = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(49 /* X_V_TEXT_NO_EXPRESSION */, loc)
      );
    }
    if (node.children.length) {
      context.onError(
        createDOMCompilerError(50 /* X_V_TEXT_WITH_CHILDREN */, loc)
      );
      node.children.length = 0;
    }
    return {
  ...
  const transformModel$1 = (dir, node, context) => {
    ...
    if (dir.arg) {
      context.onError(
        createDOMCompilerError(52 /* X_V_MODEL_ARG_ON_ELEMENT */, dir.arg.loc)
      );
    }
      ...
      if (value) {
        context.onError(
          createDOMCompilerError(
            54 /* X_V_MODEL_UNNECESSARY_VALUE */,
            value.loc
          )
        );
      }
    ...
    if (tag === "input" || tag === "textarea" || tag === "select") {
          ...
          if (type.type === 7 /* DIRECTIVE */) {
            ...
            switch (type.value.content) {
              ...
              case "file":
                ...
                context.onError(
                  createDOMCompilerError(
                    53 /* X_V_MODEL_ON_FILE_INPUT_ELEMENT */,
                    dir.loc
                  )
                );
                break;
      ...
      context.onError(
        createDOMCompilerError(51 /* X_V_MODEL_ON_INVALID_ELEMENT */, dir.loc)
      );
    }
  ...
  const transformShow = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(55 /* X_V_SHOW_NO_EXPRESSION */, loc)
      );
    }
    return {
      props: [],
      ...
      if (component === TRANSITION) {
        return () => {
          if (node.children.length && hasMultipleChildren(node)) {
            context.onError(
              createDOMCompilerError(56 /* X_TRANSITION_INVALID_CHILDREN */, {
                start: node.children[0].loc.start,
                end: node.children[node.children.length - 1].loc.end,
                source: "",
              })
    ...
    if (instance) {
      ...
      }
      {
        if (res) {
          ...
          if (type === COMPONENTS && isObject(res) && !res.name) {
            res.name = name;
          }
        } else if (warnMissing) {
  ...
  function isSameVNodeType(n1, n2) {
    if (n2.shapeFlag & 6 /* COMPONENT */ && n2.type.__hmrUpdated) {
  ...
  */
  ...
  function baseCreateRenderer(options, createHydrationFns) {
    ...
    const setRef = (rawRef, oldRawRef, parent, value) => {
      ...
      if (isString(ref)) {
        ...
        callWithErrorHandling(ref, parent, 12 /* FUNCTION_REF */, [
          value,
          refs,
        ]);
      } else {
  ...
  function renderSlot(
    ...
    return (
      ...
      createBlock(
        ...
        slots._ ? 64 /* STABLE_FRAGMENT */ : -2 /* BAIL */
      )
    );
  }
  ...
  function patchEvent(el, rawName, prevValue, nextValue, instance = null) {
    ...
    const prevOptions =
      prevValue && "options" in prevValue && prevValue.options;
    const nextOptions =
      nextValue && "options" in nextValue && nextValue.options;
